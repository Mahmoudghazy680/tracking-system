...
use App\Http\Controllers\Api\Reports\DailyEmployeeUsageReportController;
use App\Http\Controllers\Api\Reports\DailyProgramSummaryReportController;
use App\Http\Controllers\Api\Reports\TimeUseReportController;
use App\Http\Controllers\Api\TaskController;
use App\Http\Controllers\Api\IntervalController;
use App\Http\Controllers\Api\Reports\SoftwareUsageReportController;
use App\Http\Controllers\Api\Reports\TopProgramsReportController;
use App\Http\Controllers\Api\Reports\TopWebsitesReportController;
use App\Http\Controllers\Api\Reports\DashboardStatsController;
use App\Http\Controllers\Api\Reports\EmailMonitoringReportController;
use App\Http\Controllers\Api\EmailMonitoringController;
...
use App\Http\Controllers\WindowsAuthController;
...
Route::group([
    ...
    $router->withoutMiddleware('auth:sanctum')->group(static function (Router $router) {
        $router->post('login', [AuthController::class, 'login'])
            ->name('auth.login');
        $router->post('login-by-windows-user', [WindowsAuthController::class, 'login'])
            ->middleware('throttle:10,1')
            ->name('auth.login.windows_user');
        $router->post('password/reset/request', [PasswordResetController::class, 'request'])
            ->name('auth.reset.request');
        $router->post('password/reset/validate', [PasswordResetController::class, 'validate'])
            ->name('auth.reset.validate');
...
Route::group([
    ...
    $router->group(['middleware' => ['throttle:120,1']], static function (Router $router) {
        ...
        $router->post('report/software-usage', [SoftwareUsageReportController::class, '__invoke'])
            ->name('report.software-usage');
        $router->post('report/software-usage/download', [SoftwareUsageReportController::class, 'download'])
            ->name('report.software-usage.download');
        $router->post('report/daily-program-summary', [DailyProgramSummaryReportController::class, '__invoke'])
            ->name('report.daily-program-summary');
        $router->post('report/daily-program-summary/download', [DailyProgramSummaryReportController::class, 'download'])
            ->name('report.daily-program-summary.download');
        $router->post('report/daily-employee-usage', [DailyEmployeeUsageReportController::class, '__invoke'])
            ->name('report.daily-employee-usage');
        $router->post('report/daily-employee-usage/download', [DailyEmployeeUsageReportController::class, 'download'])
            ->name('report.daily-employee-usage.download');
        $router->post('report/top-programs', [TopProgramsReportController::class, '__invoke'])
            ->name('report.top-programs');
        $router->post('report/top-programs/download', [TopProgramsReportController::class, 'download'])
            ->name('report.top-programs.download');
        $router->post('report/top-websites', [TopWebsitesReportController::class, '__invoke'])
            ->name('report.top-websites');
        $router->post('report/top-websites/download', [TopWebsitesReportController::class, 'download'])
            ->name('report.top-websites.download');
        $router->post('report/dashboard-stats', [DashboardStatsController::class, '__invoke'])
            ->name('report.dashboard-stats');
        $router->post('report/email-monitoring', [EmailMonitoringReportController::class, '__invoke'])
            ->name('report.email-monitoring');
        $router->post('email-events', [EmailMonitoringController::class, 'store'])
            ->name('email-events.store');
        // About
