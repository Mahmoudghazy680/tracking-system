...
class IntervalController extends ItemController
{
    ...
    protected const TRACKED_TEXT_BIDI_REGEX = '/[\x{200E}\x{200F}\x{202A}-\x{202E}\x{2066}-\x{2069}]/u';
    ...
    public function trackApp(TrackAppRequest $request): JsonResponse
    {
        ...
        $title = $this->sanitizeTrackedText($data['title'] ?? null, 255);

        $executable = $this->sanitizeTrackedText($data['executable'] ?? null, 255, false);

        if ($executable === '') {
            $fallback = $title ? trim((string) $title) : '';
            $executable = $fallback !== '' ? $fallback : 'unknown';
        }

        $data['title'] = $title;
        $data['executable'] = $this->sanitizeTrackedText($executable, 255, false);
        $data['url'] = $this->sanitizeTrackedText($data['url'] ?? null, 2048);
    ...
    private function sanitizeTrackedText($value, int $maxLength, bool $nullable = true): ?string
    {
        ...
        $text = preg_replace(self::TRACKED_TEXT_BIDI_REGEX, '', $text) ?? $text;
        ...
        return $text === '' && $nullable ? null : $text;
    }
