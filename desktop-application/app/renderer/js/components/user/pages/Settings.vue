  ...
  <el-container class="settings-container">
    ...
    <div class="settings__footer">
      ...
      <small style="color: #999">
        PIN-Tracker {{ version.number }} = BSRD Team
        <br>
        {{ version.devMode ? '+dev ' : '' }}
        {{ version.sentry ? 'Pinnacle' : '' }}
      </small>
    </div>
  </el-container>
</template>
...
export default {
    ...
    this.$ipc.request('user-preferences/export-structure', {}).then(({ body }) => {
      ...
      Object.entries(body.preferences).forEach(([prefKey, prefVal]) => {
        ...
        const renderableField = {
          ...
          readonly: Boolean(prefVal.frontend.readonly),
          frontend: {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      const readonlyKeys = new Set(this.formFields
        .filter(field => field.readonly)
        .map(field => field.key));
      const editablePreferences = Object
        .entries(this.formData)
        .filter(([key]) => !readonlyKeys.has(key))
      ...
      if (res.code === 200) {
        ...
        if ('language' in editablePreferences)
          this.$i18n.locale = editablePreferences.language;
