...
const isDomainClientEnabled = (
  process.env.AT_DOMAIN_CLIENT === 'yes'
  || packageManifest.domainClient === true
);
...
const configuration = {
  ...
  // Is this a dedicated domain-auth client flavor?
  isDomainClientEnabled,
...
configuration.authentication = {
  ...
  defaultHostname: 'http://172.16.70.66/',
  windowsAuthSecret: process.env.AT_WINDOWS_AUTH_SECRET || packageManifest.windowsAuthSecret || '',
  domainOnlyEnabled: isDomainClientEnabled || process.env.AT_DOMAIN_ONLY_AUTH === 'yes',
  startHiddenInTray: isDomainClientEnabled || process.env.AT_START_HIDDEN_TRAY === 'yes',
  silentInactivityHandling: isDomainClientEnabled || process.env.AT_SILENT_INACTIVITY === 'yes',
  disableSso: isDomainClientEnabled || process.env.AT_DISABLE_SSO === 'yes',
