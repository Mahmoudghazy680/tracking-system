...
const preferences = {
  ...
  updateNotification: {
    ...
    description: 'Notify about PIN-Tracker updates available on launch',
    default: false,
    frontend: {
      element: 'toggle',
  ...
  usageStatistics: {
    ...
    description: 'Share usage statistics with PIN-Tracker developers. Statistics include only versions of PIN-Tracker, Operating System, and backend installation ID. We don\'t collect any personal data, and all collected statistics will be used only to better understand how we can improve the desktop application experience.',
    default: true,
    frontend: {
      element: 'toggle',
  ...
  errorReporting: {
    ...
    description: 'When an error occurs, PIN-Tracker will send an anonymized report to the development team with details concerning that error. Application restart is required to apply the new sharing policy.',
    default: true,
    frontend: {
      element: 'toggle',
  ...
  autoStartTrackingOnActivity: {
    type: 'boolean',
    name: 'Auto-start tracking on activity',
    description: 'Automatically resume tracking the last task when activity is detected while tracker is stopped.',
    default: false,
    frontend: {
      element: 'toggle',
      options: { 'Disable auto-start': false, 'Auto-start on activity': true },
    },
  },
...
class UserPreferences extends EventEmitter {
  ...
  constructor() {
    ...
    const lockedDefaultsWereApplied = this.applyLockedDefaults();
    if (lockedDefaultsWereApplied)
  ...
  /**
   * Force default values for locked preferences.
   * @return {boolean} true when any locked value has changed
   */
  applyLockedDefaults() {

    let hasChanges = false;

    LOCKED_PREFERENCE_KEYS.forEach(key => {

      if (this.preferencesFileBuffer[key] !== preferences[key].default) {
