...
class TaskTracker extends EventEmitter {
  ...
  constructor() {
    ...
    /**
     * Identifier of background activity monitor timer.
     * @type {Number|null}
     */
    this.activityAutoStartTimerId = null;

    /**
     * Last idle-time reading used to detect new input.
     * @type {Number|null}
     */
    this.activityAutoStartLastIdleSeconds = null;

    /**
     * Cooldown timestamp for auto-resume attempts.
     * @type {Number}
     */
    this.activityAutoStartCooldownUntil = 0;

    /**
     * Guard against overlapping auto-resume attempts.
     * @type {Boolean}
     */
    this.activityAutoStartInProgress = false;

    /**
     * Cooldown window after manual/inactivity stop.
     * @type {Number}
     */
    this.activityAutoStartCooldownMs = 15000;

    /**
     * Enforced tracking policy toggle.
     * @type {Boolean}
     */
    this.forceAlwaysOnTracking = true;

    /**
     * Guard against overlapping forced auto-start bootstrap requests.
     * @type {Boolean}
     */
    this.forcedBootstrapInProgress = false;

    /**
     * Retry timer for forced bootstrap when startup dependencies are delayed.
     * @type {Number|null}
     */
    this.forcedBootstrapRetryTimerId = null;

    /**
     * Delay before retrying forced bootstrap.
     * @type {Number}
     */
    this.forcedBootstrapRetryDelayMs = 10000;
    ...
    this.on('activity-proof-result', async result => {
      ...
      await this.stop(false, true, 'inactivity');
    ...
    activeWindow.on('unavailable', ({ reason }) => {

      log.warning(`App monitoring is unavailable on this device: ${reason}`);

    });

    trackingFeature.on('features-changed', features => {

      this.syncTrackingFeatures(features);

    });

    /**
     * Handle email detection events from the email monitor.
     * Forward captured email metadata to the backend.
     */
    emailMonitor.on('email-detected', async emailData => {

      try {

        await api.post('email-events', emailData);
        log.debug('Email event sent to backend');

      } catch (err) {

        log.warning('Failed to send email event to backend', err);

      }

    });

    this.startAutoStartMonitor();

    Authentication.events.on('authenticated', () => this.ensureAlwaysOnTracking('authenticated'));
    Authentication.events.on('user-fetched', () => this.ensureAlwaysOnTracking('user-fetched'));

    // Safety net in case startup auth/task events arrive out of order.
    this.scheduleAlwaysOnRetry('startup');

  }

  /**
   * Gets fallback task for always-on tracking mode.
   * @returns {Promise<Model<Task>|null>}
   */
  async resolveFallbackTask() {
    ...
    try {
      ...
      await TaskController.syncTasks(false, false, true);
      ...
      log.warning(`Cannot sync tasks during always-on bootstrap: ${error.message || error}`);
  ...
  /**
   * Ensures tracking starts automatically after authentication.
   * @param {String} source Trigger source
   */
  async ensureAlwaysOnTracking(source = 'unknown') {

    if (!this.forceAlwaysOnTracking || this.active || this.forcedBootstrapInProgress)
      return;

    this.forcedBootstrapInProgress = true;

    try {

      await this.start();
      this.clearAlwaysOnRetry();
      log.debug(`Always-on bootstrap completed from ${source}`);

    } catch (error) {

      log.warning(`Always-on bootstrap skipped (${source}): ${error.message || error}`);
      this.scheduleAlwaysOnRetry(source);
  ...
  /**
   * Schedules a retry for forced always-on bootstrap.
   * @param {String} source Trigger source
   */
  scheduleAlwaysOnRetry(source = 'unknown') {

    if (!this.forceAlwaysOnTracking || this.active || this.forcedBootstrapRetryTimerId)
      return;

    this.forcedBootstrapRetryTimerId = setTimeout(() => {

      this.forcedBootstrapRetryTimerId = null;
      this.ensureAlwaysOnTracking(`retry:${source}`);

    }, this.forcedBootstrapRetryDelayMs);

  }

  /**
   * Clears pending forced bootstrap retry timer.
   */
  clearAlwaysOnRetry() {

    if (!this.forcedBootstrapRetryTimerId)
      return;

    clearTimeout(this.forcedBootstrapRetryTimerId);
    this.forcedBootstrapRetryTimerId = null;
  ...
  /**
   * Determine whether auto-start on activity is enabled.
   * @returns {Boolean}
   */
  isAutoStartOnActivityEnabled() {

    try {

      if (this.forceAlwaysOnTracking)
        return true;

      return Boolean(userPreferences.get('autoStartTrackingOnActivity'));

    } catch (err) {

      log.warning('Cannot read auto-start preference, falling back to disabled state');
      return this.forceAlwaysOnTracking;
  ...
  /**
   * Starts background monitor to resume the previous task on fresh user activity.
   */
  startAutoStartMonitor() {

    if (this.activityAutoStartTimerId)
      return;

    this.activityAutoStartLastIdleSeconds = powerMonitor.getSystemIdleTime();

    this.activityAutoStartTimerId = setInterval(async () => {

      if (this.active || this.activityAutoStartInProgress)
        return;

      if (!this.previousTask)
        return;

      if (!this.isAutoStartOnActivityEnabled())
        return;

      if (Date.now() < this.activityAutoStartCooldownUntil)
        return;

      const currentIdleSeconds = powerMonitor.getSystemIdleTime();
      const previousIdleSeconds = this.activityAutoStartLastIdleSeconds;
      this.activityAutoStartLastIdleSeconds = currentIdleSeconds;

      // A decreasing idle counter means new input was detected.
      const hasFreshActivity = (
        typeof previousIdleSeconds === 'number'
        && currentIdleSeconds < previousIdleSeconds
      ...
      this.activityAutoStartInProgress = true;

      try {

        await this.start();
        this.emit('auto-resumed', { task: this.previousTask?.id || null });
        log.debug('Tracking resumed automatically due to detected user activity');

      } catch (error) {

        log.warning(`Auto-resume attempt failed: ${error.message || error}`);
  ...
  /**
   * Stops background auto-start monitor.
   */
  stopAutoStartMonitor() {

    if (!this.activityAutoStartTimerId)
      return;

    clearInterval(this.activityAutoStartTimerId);
    this.activityAutoStartTimerId = null;
    this.activityAutoStartLastIdleSeconds = null;
    this.activityAutoStartInProgress = false;
  ...
  async start(taskId) {
    ...
    if (typeof taskId !== 'string') {
      ...
      if (!this.previousTask) {
        ...
        this.previousTask = await this.resolveFallbackTask();

        if (!this.previousTask)
          throw new UIError(500, 'Tracker re-start request rejected due to lack of available tasks');
      ...
      if (this.active && this.currentTask) {
        ...
        await this.stop(true, false, 'switch');
    ...
    // Suspend background activity monitor while tracker is running.
    this.stopAutoStartMonitor();

    // Dispatch corresponding event
    this.setTrackerStatus(true);
    // Enable monitoring features once tracker runtime is active.
    const trackingFeatures = await trackingFeature.getCurrentFeatures() || [];
    this.syncTrackingFeatures(trackingFeatures);
  ...
  /**
   * Apply runtime monitoring services based on persisted tracking features.
   * @param {string[]} trackingFeatures
   */
  syncTrackingFeatures(trackingFeatures = []) {

    if (!this.active)
      return;

    const shouldTrackApplications = trackingFeatures.includes('APP_MONITORING');
    const shouldTrackEmail = trackingFeatures.includes('EMAIL_MONITORING');
    const needsActiveWindow = shouldTrackApplications || shouldTrackEmail;

    if (needsActiveWindow) {

      if (!activeWindow.active && !activeWindow.start()) {

        const disableReason = activeWindow.getDisableReason();
        if (disableReason)
          log.warning(`Active window monitoring is disabled (${disableReason}); time tracking will continue`);
        else
          log.warning('Active window monitoring is disabled; time tracking will continue');

      }

    } else if (activeWindow.active) {

      activeWindow.stop();

    }

    if (shouldTrackEmail && activeWindow.active) {

      if (!emailMonitor.active)
        emailMonitor.start(activeWindow);

    } else if (emailMonitor.active) {
   ...
   */
  async stop(pushInterval = true, emitEvent = true, stopReason = 'system') {

    if (this.forceAlwaysOnTracking && stopReason === 'manual')
      throw new UIError(403, 'Tracking stop is restricted by organization policy', 'ERTR403');
    ...
    if (stopReason === 'manual' || stopReason === 'inactivity')
      this.activityAutoStartCooldownUntil = Date.now() + this.activityAutoStartCooldownMs;
