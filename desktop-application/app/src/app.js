...
if (!config.isDeveloperModeEnabled) {
  ...
  if (app.requestSingleInstanceLock()) {
    ...
    app.on('second-instance', () => {
      ...
      if (!window)
        return;
      if (config.authentication.startHiddenInTray) {
...
app.once('ready', async () => {
  ...
  // Attach IPC target before renderer bootstraps so early startup
  // requests (e.g. translations/auth state) cannot deadlock.
  ...
  // Domain clients still use the renderer for task selection and status.
  // Authentication is performed automatically by the normal auth route.
  loadPage('app.html');
