...
const activeWindow = require('../base/active-window');
...
class TrackingFeatures extends EventEmitter {
  ...
  static isAppMonitoringEnabled(user) {

    return Boolean(user && (
      user.appMonitoringEnabled
      || user.web_and_app_monitoring
    ));
  ...
  static parseUserFeatures(user) {
    ...
    if (TrackingFeatures.isAppMonitoringEnabled(user)) {

      if (activeWindow.canMonitor()) {
        features.push('APP_MONITORING');
        features.push('EMAIL_MONITORING');
      } else {
        log.warning(`App monitoring is disabled on this machine: ${activeWindow.getDisableReason() || 'unknown reason'}`);
      }
