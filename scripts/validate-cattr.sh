...
status_code="$(curl -sS -o /tmp/Tracker-status.json -w '%{http_code}' "$api_base/status")"
if [[ "$status_code" -lt 200 || "$status_code" -ge 400 ]]; then
  echo "Status endpoint failed with HTTP $status_code" >&2
  cat /tmp/Tracker-status.json >&2 || true
  exit 1
fi

echo "Attempting admin login"
login_code="$(curl -sS -o /tmp/Tracker-login.json -w '%{http_code}' -X POST "$api_base/auth/login" \
...
if [[ "$login_code" -lt 200 || "$login_code" -ge 400 ]]; then
  ...
  cat /tmp/Tracker-login.json >&2 || true
  exit 1
fi

token="$(jq -r '.data.access_token // .access_token // .token // empty' /tmp/Tracker-login.json)"

if [[ -z "$token" ]]; then
  echo "Login response did not include a bearer token" >&2
  cat /tmp/Tracker-login.json >&2
  exit 1
fi

echo "Checking authenticated user"
me_code="$(curl -sS -o /tmp/Tracker-me.json -w '%{http_code}' "$api_base/auth/me" \
  -H "Authorization: Bearer $token")"

if [[ "$me_code" -lt 200 || "$me_code" -ge 400 ]]; then
  echo "Auth me failed with HTTP $me_code" >&2
  cat /tmp/Tracker-me.json >&2 || true
  exit 1
fi

echo "Validation succeeded"
echo "Status response:"
cat /tmp/Tracker-status.json
echo
echo "Authenticated user response:"
cat /tmp/Tracker-me.json
echo
